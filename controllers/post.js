...
const incrementQuestionCount = async (userId) => {
  try {
    await User.findByIdAndUpdate(userId, { $inc: { questionsPostedToday: 1 } });
  } catch (error) {
    console.error("Error incrementing question count:", error);
  }
};

// Function to reset the questionsPostedToday count for all users
const resetQuestionCount = async () => {
  try {
    await User.updateMany({}, { $set: { questionsPostedToday: 0 } });
  } catch (error) {
...
const resetQuestionCountScheduler = () => {
  resetQuestionCount();
  setInterval(resetQuestionCount, 30 * 24 * 60 * 60 * 1000); // Run every 30 days
};
